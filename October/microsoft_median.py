...
starting from the left and moving right by one position each time.
...
def medians(arr, k):
    for x in range(len(arr)-(k-1)):
        ...
        window=sorted(arr[x:end])
        

        print(f'{window[1]} <- median of {arr[x:end]}')


test_k=3
test_arr=[-1, 5, 13, 8, 2, 3, 3, 1]
medians(test_arr, test_k)